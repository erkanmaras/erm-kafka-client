...
public class SecurityInformation
{
    ...
    /// <summary>
    ///     Gets or sets the SASL mechanism to use for authentication. Supported: GSSAPI, PLAIN, SCRAM-SHA-256, SCRAM-SHA-512.
    ///     **NOTE**: Despite the name, you may not configure more than one mechanism.
    /// </summary>
    public SaslMechanism? SaslMechanism { get; set; }

    /// <summary>
    ///     Gets or sets the cipher suites. A cipher suite is a named combination of authentication, encryption, MAC and key
    ///     exchange algorithm used to negotiate the security settings for a network connection using TLS or SSL network
    ///     protocol. See manual page for `ciphers(1)` and `SSL_CTX_set_cipher_list(3).
    ///     default: ''
    ///     importance: low
    /// </summary>
    ...
    /// <summary>
    ///     Gets or sets the supported-curves extension in the TLS ClientHello message specifies the curves (standard/named, or
    ...
    /// <summary>
    ///     Gets or sets the sigalgs list. The client uses the TLS ClientHello signature_algorithms extension to indicate to
    ///     the server which signature/hash algorithm pairs may be used in digital signatures. See manual page for
    ...
    /// <summary>
    ///     Gets or sets the Enable OpenSSL's builtin broker (server) certificate verification. This verification can be
    ///     extended by the application by implementing a certificate_verify_cb.
    ///     default: true
    ///     importance: low
    /// </summary>
    ...
    /// <summary>
    ///     Gets or sets the endpoint identification algorithm to validate broker hostname using broker certificate. https -
    ///     Server (broker) hostname verification as specified in RFC2818. none - No endpoint verification. OpenSSL &gt;= 1.0.2
    ///     required.
    ///     default: none
    ///     importance: low
    /// </summary>
    ...
    /// <summary>
    ///     Gets or sets the shell command to refresh or acquire the client's Kerberos ticket. This command is executed on
    ///     client creation and every sasl.kerberos.min.time.before.relogin (0=disable). %{config.prop.name} is replaced by
    ///     corresponding config object value.
    ///     default: kinit -R -t "%{sasl.kerberos.keytab}" -k %{sasl.kerberos.principal} || kinit -t "%{sasl.kerberos.keytab}"
    ...
    /// <summary>
    ///     Gets or sets the path to Kerberos keytab file. This configuration property is only used as a variable in
    ...
    /// <summary>
    ///     Gets or sets the minimum time in milliseconds between key refresh attempts. Disable automatic key refresh by
    ...
    /// <summary>
    ///     Gets or sets the SASL/OAUTHBEARER configuration. The format is implementation-dependent and must be parsed
    ///     accordingly. The default unsecured token implementation (see https://tools.ietf.org/html/rfc7515#appendix-A.5)
    ///     recognizes space-separated name=value pairs with valid names including principalClaimName, principal,
    ///     scopeClaimName, scope, and lifeSeconds. The default value for principalClaimName is "sub", the default value for
    ///     scopeClaimName is "scope", and the default value for lifeSeconds is 3600. The scope value is CSV format with the
    ///     default value being no/empty scope. For example: `principalClaimName=azp principal=admin scopeClaimName=roles
    ///     scope=role1,role2 lifeSeconds=600`. In addition, SASL extensions can be communicated to the broker via
    ///     `extension_NAME=value`. For example: `principal=admin extension_traceId=123`
    ///     default: ''
    ///     importance: low
    /// </summary>
    ...
    /// <summary>
    ///     Gets or sets enable the builtin unsecure JWT OAUTHBEARER token handler if no oauthbearer_refresh_cb has been set.
