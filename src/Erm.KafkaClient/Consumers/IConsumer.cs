...
/// <summary>
/// Represents a KafkaClient consumer
/// </summary>
public interface IConsumer : IDisposable
{
    ...
    void OnPartitionsAssigned(Action<IServiceProvider, IConsumer<byte[], byte[]>, List<TopicPartition>> handler);

    /// <summary>
    /// Register a handler to be executed when the partitions are revoked
    /// </summary>
    /// <param name="handler">The handler that will be executed</param>
    void OnPartitionsRevoked(Action<IServiceProvider, IConsumer<byte[], byte[]>, List<TopicPartitionOffset>> handler);
